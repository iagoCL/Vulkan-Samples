...
=== xref:./{api_samplespath}fill_buffer_linear_image/README.adoc[Fill buffer into a linear image]

Educational sample that shows how to pack a `glm::vec3` grid into `uint32_t` fill words, write them into aliased linear image memory with `vkCmdFillBuffer`, and copy the result to the swapchain.
